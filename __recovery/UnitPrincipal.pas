...
type
  TFrmPrincipal = class(TForm)
    ...
    VertScrollBoxProdex: TVertScrollBox;
  ...
  private
    { Private declarations }
       ...
       bairro: string; usuarioId: String);
...
{$R *.fmx}
 uses UnitDM, UnitLoginUsuario, UnitFrame, UnitEndereco;


 procedure TFrmPrincipal.CarregaListaEnderecos(id: integer; rua: string;
  bairro: string;usuarioId: String);
  var
  frame: TFrameEndereco;
  begin

    frame := TFrameEndereco.Create(nil);
    frame.Position.Y := 99999999;
    frame.Align := TAlignLayout.Top;
    frame.tag := id;
    IdEndereco := frame.tag;
    frame.lblbairro.text := bairro;
    frame.lblRua.text := rua;
   frame.imageeditar.tag := id;

    FrmPrincipal.VertScrollBoxMeusEnderecos.AddObject(frame);
  end;
  ...
  begin
    ...
    begin
      ...
      dm.FDQueryEnderecousuarioId.AsString);
...
dm.FDQueryBuscaUsuario.refresh();
...
dm.FDQueryDeleteEnd.ParamByName('pID').Asinteger := FrmPrincipal.IdEndereco ;

if(dm.FDQueryDeleteEnd = nil) then
begin
dm.FDQueryUsuario.Edit;
